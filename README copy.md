<div align="center">

<!--
  ╔══════════════════════════════════════════════════════╗
  ║  TANISHA KUSHWAH — GitHub Profile README             ║
  ║  Inspired by GrahamTheDev's visual README style      ║
  ║                                                      ║
  ║  SETUP:                                              ║
  ║  1. Upload hero.svg to this repo root                ║
  ║  2. Upload your photo as my-photo.png to repo root   ║
  ║  3. In hero.svg, replace the TK avatar with:         ║
  ║     <image href="my-photo.png" x="490" y="110"       ║
  ║       width="160" height="160"                       ║
  ║       clip-path="url(#avatarClip)"/>                 ║
  ╚══════════════════════════════════════════════════════╝
-->

<!-- ── SOCIAL BUTTONS ROW (like GrahamTheDev's top bar) ── -->
<a href="https://twitter.com/"><img height="36" src="https://img.shields.io/badge/-X%20%2F%20Twitter-000000?style=for-the-badge&logo=x&logoColor=white&labelColor=000000" /></a>&nbsp;
<a href="https://linkedin.com/in/tanisha-kushwah-280944284"><img height="36" src="https://img.shields.io/badge/-LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" /></a>&nbsp;
<a href="https://tanikush.github.io/portfolio/"><img height="36" src="https://img.shields.io/badge/-Portfolio-cc1170?style=for-the-badge&logo=vercel&logoColor=white" /></a>&nbsp;
<a href="https://youtube.com/"><img height="36" src="https://img.shields.io/badge/-YouTube-FF0000?style=for-the-badge&logo=youtube&logoColor=white" /></a>&nbsp;
<a href="#"><img height="36" src="https://img.shields.io/badge/-Twitch-9146FF?style=for-the-badge&logo=twitch&logoColor=white" /></a>

<br/><br/>

<!-- ── HERO SVG (speech bubble + photo + projects) ── -->
<!-- Upload hero.svg to this repo and it renders here -->
<img src="./hero.svg" width="100%" alt="Tanisha Kushwah — DevOps Engineer building real infrastructure on AWS"/>

</div>

---

<br/>

<table width="100%">
<tr>
<td width="50%" valign="top">

### 🛡️ VibeScan &nbsp; [![Explore](https://img.shields.io/badge/Explore-%238b2fc9?style=flat-square)](https://github.com/tanikush/vibescan)

AI Code Security Scanner that detects vulnerabilities *specifically introduced by AI coding tools* — patterns TruffleHog and GitLeaks miss completely.

`Python` &nbsp; `Security` &nbsp; `Open Source`

</td>
<td width="50%" valign="top">

### 📊 DORA Dashboard &nbsp; [![View](https://img.shields.io/badge/View-%238b2fc9?style=flat-square)](https://github.com/tanikush/dora-pipeline)

Real-time DevOps metrics — Deployment Frequency, Lead Time and Change Failure Rate. Built on React 19 + AWS Lambda + DynamoDB.

`React 19` &nbsp; `AWS Lambda` &nbsp; `DynamoDB`

</td>
</tr>
<tr>
<td width="50%" valign="top">

### 🏥 MediWatch &nbsp; [![View](https://img.shields.io/badge/View-%238b2fc9?style=flat-square)](https://github.com/tanikush/MediWatch)

Hospital Reliability Platform with K8s + Jenkins + Prometheus. Automated infrastructure with self-healing systems.

`Kubernetes` &nbsp; `Jenkins` &nbsp; `Prometheus`

</td>
<td width="50%" valign="top">

### ⚡ Multi-Region CI/CD &nbsp; [![View](https://img.shields.io/badge/View-%238b2fc9?style=flat-square)](https://github.com/tanikush/devops-aws-multiregion-pipeline)

Enterprise serverless pipeline on AWS Free Tier. Multi-region architecture with automated disaster recovery built-in.

`AWS` &nbsp; `Terraform` &nbsp; `Serverless`

</td>
</tr>
</table>

---

<div align="center">

**Cloud & Infra** &nbsp;
![AWS](https://img.shields.io/badge/AWS-FF9900?style=flat-square&logo=amazon-aws&logoColor=white)
![Terraform](https://img.shields.io/badge/Terraform-5835CC?style=flat-square&logo=terraform&logoColor=white)
![Kubernetes](https://img.shields.io/badge/Kubernetes-326ce5?style=flat-square&logo=kubernetes&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
&nbsp;**CI/CD** &nbsp;
![GitHub Actions](https://img.shields.io/badge/GH%20Actions-2088FF?style=flat-square&logo=github-actions&logoColor=white)
![Jenkins](https://img.shields.io/badge/Jenkins-D24939?style=flat-square&logo=jenkins&logoColor=white)
![Ansible](https://img.shields.io/badge/Ansible-EE0000?style=flat-square&logo=ansible&logoColor=white)
&nbsp;**Observability** &nbsp;
![Prometheus](https://img.shields.io/badge/Prometheus-E6522C?style=flat-square&logo=prometheus&logoColor=white)
![Grafana](https://img.shields.io/badge/Grafana-F46800?style=flat-square&logo=grafana&logoColor=white)

<br/>

<img height="165" src="https://github-readme-stats.vercel.app/api?username=tanikush&show_icons=true&theme=radical&hide_border=true&bg_color=0d0f1a&title_color=ee3399&icon_color=8b2fc9&text_color=ffffff"/>
&nbsp;
<img height="165" src="https://github-readme-stats.vercel.app/api/top-langs/?username=tanikush&layout=compact&theme=radical&hide_border=true&bg_color=0d0f1a&title_color=ee3399&text_color=ffffff"/>

<br/>

[![GitHub Streak](https://streak-stats.demolab.com?user=tanikush&theme=radical&hide_border=true&background=0d0f1a&ring=ee3399&fire=8b2fc9&currStreakLabel=ffffff)](https://git.io/streak-stats)

<br/><br/>

*"Infrastructure that scales. Code that ships. Systems that heal themselves."*
&nbsp;·&nbsp; 🎯 **Goal: Cloud Solution Architect** &nbsp;·&nbsp; 📍 India &nbsp;·&nbsp; 💼 Open to DevOps Internships

</div>
